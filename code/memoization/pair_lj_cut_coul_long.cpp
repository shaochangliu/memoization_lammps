...
// Define the key type
struct Key_fpair {
    int itype, jtype, factor_lj, factor_coul;
    double qi, qj, rsq;

    bool operator<(const Key_fpair &other) const {
        return std::tie(itype, jtype, qi, qj, rsq, factor_lj, factor_coul) <
               std::tie(other.itype, other.jtype, other.qi, other.qj, other.rsq, other.factor_lj, other.factor_coul);
    }
};

// Refer to the lookup table
extern std::map<Key_fpair, double> lookup_table_fpair;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if ((!eflag) && (!evflag)) {
          ...
          if (qtmp < q[j])
              k = {itype, jtype, factor_lj, factor_coul, qtmp, q[j], rsq};
          else
              k = {jtype, itype, factor_lj, factor_coul, q[j], qtmp, rsq};
          
          auto it = lookup_table_fpair.find(k);
          if (it != lookup_table_fpair.end()) {
              // If the key is found in the lookup table, use the stored value
              fpair = it->second;
          } else {
              // Otherwise, calculate fpair and store it in the lookup table
              r2inv = 1.0/rsq;
