...
void PairLJCutCoulLong::compute(int eflag, int vflag)
{
  ...
  // top-k neighbors, k=10
  for (ii = 0; ii < inum; ii++){
    i = ilist[ii];
    jlist = firstneigh[i];
    jnum = numneigh[i];
    if (jnum >= 10) {
      outfile_f << q[i] << ", " << type[i] << ", " << x[i][0] << ", " << x[i][1] << ", " << x[i][2] << ", ";
      for (jj = 0; jj < 10; jj++){
        j = jlist[jj];
        j &= NEIGHMASK;
        outfile_f << q[j] << ", " << type[j] << ", " << x[j][0] << ", " << x[j][1] << ", " << x[j][2] << ", ";
      }
      outfile_f << f[i][0] << ", " << f[i][1] << ", " << f[i][2] << std::endl;
    }
  }
