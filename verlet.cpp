...
void Verlet::run(int n)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    // PairLJCutCoulLong src\KSPACE\pair_lj_cut_coul_long.h
    ...
    if (atom->molecular != Atom::ATOMIC) {
      // BondHarmonic src\MOLECULE\bond_harmonic.h
      if (force->bond) force->bond->compute(eflag,vflag);
      // AngleHarmonic src\MOLECULE\angle_harmonic.h
      if (force->angle) force->angle->compute(eflag,vflag);
      // undefined
      if (force->dihedral) force->dihedral->compute(eflag,vflag);
      // undefined
    ...
    // fix_ehex, fix_rattle, fix_ave_chunk, fix_ave_time
